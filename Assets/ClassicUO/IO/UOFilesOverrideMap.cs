...
namespace ClassicUO.IO
{
    internal class UOFilesOverrideMap : Dictionary<string, string>
    {
        ...
        internal void Load()
        {
            ...
            {
                UnityEngine.Debug.Log($"No Override File found, ignoring.");
                return; // if the file doesn't exist then we ignore
            }

            UnityEngine.Debug.Log($"Loading Override File:\t\t{OverrideFile}");
                ...
                while (!reader.EndOfStream)
                {
                    try
                    {
                        ...
                        if (segments.Length == 2)
                        {
                            ...
                            UnityEngine.Debug.Log($"Override entry: {file} => {filePath}.");
                    ...
                    {
                        UnityEngine.Debug.LogWarning("Something went wrong when trying to parse UOFileOverride file.");
